...
- **Subscription System**: A two-tier membership model (Advanced ₩3,900/월 → 할인가 ₩1,900/월, Premium ₩6,900/월). Advanced offers photo 500장, 커플 통계, 고급 캘린더, 스마트 알림. Premium is currently "coming soon" (UI hidden) - will include AI 추천, 무제한 저장, 히스토리 북 등. Annual subscription gets 20% discount. Premium feature pages (AI recommend, couple stats) exist but are hidden from dashboards until ready. Wedding mode features are not tied to this subscription. Widget store offers basic widgets (D-Day, 캘린더, 할일) and premium widgets (무드싱크, 러브레터, 기념일 리마인더, 포토 슬라이드) - all in-app dashboard cards, not OS-level home screen widgets.
