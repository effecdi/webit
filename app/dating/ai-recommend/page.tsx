...
const MOOD_OPTIONS = ["로맨틱", "활동적", "힐링", "맛집 탐방", "문화/예술", "자연"]
const BUDGET_OPTIONS = ["5만원 이하", "5-10만원", "10-20만원", "제한 없음"]

export default function AIDateRecommendPage() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      const res = await fetch("/api/ai/date-recommend", {
        ...
        body: JSON.stringify({ location: location || undefined, mood: mood || undefined, budget: budget || undefined }),
      })
    ...
    return (
      <main className="min-h-dvh bg-[#F7F8FA] dark:bg-[#0D0D0D]">
        <div className="flex items-center justify-center h-screen">
          <Loader2 className="w-8 h-8 text-[#d63bf2] animate-spin" />
        </div>
        <DatingBottomNav />
      </main>
    )
      ...
      <main className="min-h-dvh bg-[#F7F8FA] dark:bg-[#0D0D0D]">
        <header className="sticky top-0 z-50 bg-white/80 dark:bg-[#1A1A1A]/80 backdrop-blur-md border-b border-[#E5E8EB] dark:border-gray-800">
          <div className="flex items-center px-4 h-14 max-w-md mx-auto">
            ...
            <h1 className="flex-1 text-center text-[17px] font-bold text-[#191F28] dark:text-white">AI 데이트 추천</h1>
            <div className="w-9" />
          </div>
        </header>
        <div className="flex flex-col items-center justify-center px-8 pt-32 text-center max-w-md mx-auto">
          <div className="w-20 h-20 bg-purple-100 dark:bg-purple-900/30 rounded-full flex items-center justify-center mb-6">
            <Lock className="w-10 h-10 text-[#d63bf2]" />
          </div>
          ...
          <p className="text-[14px] text-[#8B95A1] dark:text-gray-400 mb-8 leading-relaxed">
            프리미엄 멤버십을 구독하면<br />AI가 맞춤 데이트 코스를 추천해드려요
          </p>
          <button
  ...
  return (
    <main className="min-h-dvh bg-[#F7F8FA] dark:bg-[#0D0D0D] pb-24">
      ...
      <div className="px-5 py-6 max-w-md mx-auto space-y-5">
        <div className="bg-gradient-to-br from-[#d63bf2] to-[#a855f7] rounded-[20px] p-5 text-white">
          <div className="flex items-center gap-2 mb-2">
            <Sparkles className="w-5 h-5" />
            <span className="text-[15px] font-bold">AI 맞춤 데이트 코스</span>
          </div>
          <p className="text-[13px] opacity-80">
            원하는 분위기와 예산을 선택하면 AI가 딱 맞는 데이트 코스를 추천해드려요
          </p>
        </div>
        ...
        <div className="bg-white dark:bg-[#1A1A1A] rounded-[16px] p-5 shadow-sm space-y-4">
          ...
          <div>
            <label className="text-[13px] font-semibold text-[#4E5968] dark:text-gray-300 mb-2 block">분위기</label>
          ...
          <div>
            <label className="text-[13px] font-semibold text-[#4E5968] dark:text-gray-300 mb-2 block">예산</label>
            <div className="flex flex-wrap gap-2">
        ...
        {hasGenerated && recommendations.length > 0 && (
          <div className="space-y-4">
            <h3 className="text-[16px] font-bold text-[#191F28] dark:text-white flex items-center gap-2">
              <Sparkles className="w-4 h-4 text-[#d63bf2]" />
              추천 데이트 코스
            </h3>
            {recommendations.map((rec, idx) => (
              ...
              >
                <div className="flex items-start justify-between mb-3">
                  ...
                  {rec.mood && (
                    <span className="px-2 py-1 bg-[#d63bf2]/10 text-[#d63bf2] text-[11px] font-medium rounded-full">
                      {rec.mood}
                    </span>
                  )}
                </div>
                ...
                <div className="space-y-2 mb-3">
                  {rec.places.map((place, pIdx) => (
                    <div key={pIdx} className="flex items-center gap-2">
                      <MapPin className="w-3.5 h-3.5 text-[#d63bf2] flex-shrink-0" />
                ...
                {rec.tip && (
                  <div className="flex items-start gap-2 mt-3 p-3 bg-amber-50 dark:bg-amber-900/20 rounded-[10px]">
                    <Lightbulb className="w-4 h-4 text-amber-500 flex-shrink-0 mt-0.5" />
                    <p className="text-[12px] text-amber-700 dark:text-amber-300">{rec.tip}</p>
                  </div>
                )}
              </div>
            ))}
